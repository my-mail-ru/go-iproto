...
- `sql.Null[T]` и legacy `sql.NullXXX` — tagWidth = 1 (`optional` префикс) + tagWidth(внутреннего типа значения)
...
> **Важно**: типы `sql.Null*` без тега `optional` обрабатываются как обычные структуры (поля `Value`/`V` + `Valid`). В этом режиме задать формат внутренних полей (длину строки, точность числа и т.д.) через промоушн тегов невозможно, поскольку `sql.Null*` как структура имеет tagWidth = 0.
