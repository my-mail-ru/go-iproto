...
Указатели и типы `sql.Null*` можно сериализовать с байтом присутствия при помощи тега `optional`:
- `0` — значение отсутствует (занимает 1 байт)
- `1` — значение присутствует, за ним следует закодированное представление (занимает 1 + длина значения байт)
