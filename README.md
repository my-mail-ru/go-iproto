...
- Для целочисленных типов неопределённого размера (`int` и `uint`) поддерживается только формат BER.
...
Обязательно:
- Понятные ошибки (по возможности, писать номер строки, эта инфромация есть в ast)
